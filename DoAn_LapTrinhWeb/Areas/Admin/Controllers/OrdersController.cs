    ...
    public class OrdersController : BaseController
    {
        ...
        public ActionResult OrderDetails(int? id)
        {
            if (User.Identity.IsAuthenticated)
            {
                ...
                User.Identity.Permiss_Modify() == true || User.Identity.Permiss_Delete() == true)
                {
                    ...
                    {
                        Notification.set_flash("Không tồn tại đơn hàng: " + "NVT" + id + ")", "warning");
                    ...
                    ProductOrder += "<tr style='border-bottom: 1px solid rgba(0,0,0,.05);' class='product_item'>" +
                            "<td valign='middle' width ='80%' style='text-align:left; padding:0 2.5em;'> " +
                                "<div class='product-entry'>" +
                                    "<img src='https://nvtcomputer.tech/" + item.Product.image + "' style = 'width: 100px; max-width: 600px; height: auto; padding-bottom:5px; display: block;'>" +
                                    "<div class='text'>" +
            ...
            if (User.Identity.IsAuthenticated)
            {
                if (User.Identity.Permiss_Update() == true || User.Identity.Permiss_Modify() == true)
                {
                    ...
                    if (order != null)
                    {
                        ...
                        db.Entry(order).State = EntityState.Modified;
                        db.SaveChanges();
                        Notification.set_flash("Đã chuyển trạng thái đơn hàng: " + "NVT" + id + " sang chờ xử lý!", "success");
                    }
                    else
                    {
        ...
        string OrderPhone,string OrderName,string OrderAddress,string OrderDelivery,string DeliveryId, string emailFor)
        {
            ...
            else if (emailFor == "ChangeComplete")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được giao thành công. Cảm ơn bạn đã tin tưởng NVTComputer, hi vọng trong tương lai bạn sẽ tiếp tục sử dụng dịch vụ của Chúng tôi.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink); 
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
            ...
            else if (emailFor == "CancleOrders")
            {
                ...
                body = body.Replace("{{OrderId}}", OrderID);
                body = body.Replace("{{BodyContent}}", "Đơn hàng <span style='color: rgb(1, 181, 187); font-weight: 500;'>#" + OrderID + "</span> đã được hủy theo yêu cầu của bạn. NVTComputer bắt đầu tiến hành thủ tục hoàn tiền và sẽ thông tin đến bạn khi đã hoàn tất.");
                body = body.Replace("{{ButtonConfirm}}", "Quản lý đơn hàng");
                body = body.Replace("{{ButtonConfirmLink}}", ButtonConfirmlink);
                body = body.Replace("{{OrderStatus}}", OrderStatus);
                body = body.Replace("{{UserEmail}}", emailID);
